...
function safeUser(row) {
  if (!row) return null;
  return { id: row.id, username: row.username, fullName: row.full_name || row.fullName || '', contact: row.contact, role: row.role, permissions: row.permissions || [], createdAt: row.created_at || row.createdAt };
}
...
async function initDb() {
  ...
  await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS full_name VARCHAR(120) NOT NULL DEFAULT ''`);
  ...
  const ownerFullName = cleanText(process.env.OWNER_NAME || 'مالك ليكس ترست', 120);
  ...
  if (!existing.rowCount) {
    await pool.query(`INSERT INTO users(username,full_name,password_hash,contact,role,permissions) VALUES($1,$2,$3,$4,'owner',$5)`, [ownerUsername, ownerFullName, hash, ownerContact, PERMISSIONS]);
  } else if (process.env.SYNC_OWNER_FROM_ENV === 'true') {
    await pool.query(`UPDATE users SET username=$1,full_name=$2,password_hash=$3,contact=$4,permissions=$5,updated_at=NOW() WHERE id=$6`, [ownerUsername, ownerFullName, hash, ownerContact, PERMISSIONS, existing.rows[0].id]);
  }
}
...
app.post('/api/register', async (req, res, next) => {
  try {
    ...
    if (username.length < 3 || !fullName || password.length < 10 || !contact) return res.status(400).json({ error: 'أدخل الاسم الكامل، واسم مستخدم 3 أحرف على الأقل، وكلمة مرور 10 أحرف على الأقل، ورقم الجوال أو البريد.' });
    const passwordHash = await bcrypt.hash(password, 12);
    const q = await pool.query(`INSERT INTO users(username,full_name,password_hash,contact,role,permissions) VALUES($1,$2,$3,$4,'client','{}') RETURNING *`, [username, fullName, passwordHash, contact]);
...
async function decorateCase(row, user) {
  ...
  return {
    ...
    client:{id:row.client_id,username:row.client_username,fullName:row.client_full_name||'',contact:row.client_contact,role:row.client_role,permissions:row.client_permissions||[],createdAt:row.client_created_at},
    assignee:row.assigned_to?{id:row.assigned_to,username:row.assignee_username,fullName:row.assignee_full_name||'',contact:row.assignee_contact,role:row.assignee_role,permissions:row.assignee_permissions||[],createdAt:row.assignee_created_at}:null,
    ownerNotes:notes,
